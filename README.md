**Proyecto:** Bitácora (Agenda System)
**Base de datos:** bitacora (PostgreSQL)
**Usuario analizado:** José Guadalupe Álvarez (per_gth_id: 13010)
**Total de tareas:** 270
**Fecha de análisis:** Agosto 2026

---

## Nivel 1: Servicio → Nivel 2: Tipo de Tarea

| # | **Servicio (Nivel 1)** | **Tipo de Tarea (Nivel 2)** | Cant. |
|---|------------------------|----------------------------|-------|
| **1** | **Soporte de Correo** | | **66** |
| | | Configuración de correo | |
| | | Correo no funciona (no envía/recibe) | |
| | | Bandeja llena/atascada | |
| | | Crear cuenta de correo | |
| | | Corte de correo | |
| | | Vaciar correo | |
| | | Correo pide clave | |
| | | Seguimiento de correo | |
| **2** | **Accesos y Credenciales** | | **21** |
| | | Facilitar datos de acceso | |
| | | Contraseña/clave | |
| | | Permisos | |
| | | Datos de CRM/Zoho | |
| **3** | **Compras y Gestión** | | **21** |
| | | Pedidos de equipo | |
| | | Cotizaciones | |
| | | Seguimiento a pedidos | |
| | | Licencias | |
| **4** | **Seguimiento y Asesoría** | | **18** |
| | | Revisión de temas | |
| | | Seguimiento a incidencias | |
| | | Asesoría | |
| **5** | **Gestión de Personal** | | **15** |
| | | Horarios | |
| | | Vacaciones | |
| | | Incidencias | |
| | | Checadas | |
| **6** | **Mantenimiento y Respaldo** | | **13** |
| | | Respaldos | |
| | | Rsync/sincronización | |
| | | Compresión de respaldos | |
| **7** | **Pruebas** | | **13** |
| | | Pruebas de sistema | |
| | | Títulos de ejemplo | |
| **8** | **Soporte Contpaq** | | **9** |
| | | Actualizar Contpaq | |
| | | Contpaq no funciona | |
| | | Instalar Contpaq | |
| **9** | **Fallas y Errores** | | **9** |
| | | Conexión fallida | |
| | | Equipo no funciona | |
| | | Error de software | |
| **10** | **Capacitación** | | **8** |
| | | Cursos | |
| | | Video capacitación | |
| **11** | **Soporte General** | | **23** |
| | | Tareas misceláneas | |
| | | Apoyo a otros | |
| **12** | **Desarrollo** | | **7** |
| | | Crear sistema | |
| | | API | |
| | | Página web | |
| **13** | **Red y Conectividad** | | **6** |
| | | Internet | |
| | | VPN | |
| | | WiFi | |
| **14** | **Configuración** | | **6** |
| | | Configurar equipo | |
| | | Configurar servicio | |
| **15** | **Instalación** | | **5** |
| | | Instalar software | |
| | | Instalar hardware | |
| **16** | **Cámaras y Seguridad** | | **4** |
| | | Cámaras | |
| | | Cortinas | |
| **17** | **CRM/Zoho** | | **3** |
| | | CRM | |
| | | Zoho | |
| **18** | **Dominio y Hosting** | | **3** |
| | | Dominio | |
| | | Hosting | |
| **19** | **Servidores y BD** | | **3** |
| | | Servidor | |
| | | Base de datos | |
| **20** | **Facturación/SAT** | | **3** |
| | | XML | |
| | | Facturas | |
| | | Declaraciones | |
| **21** | **Office** | | **3** |
| | | Excel | |
| | | Word | |
| **22** | **Impresoras** | | **1** |
| | | Impresora de tickets | |
| **23** | **Antivirus** | | **2** |
| | | Deshabilitar antivirus | |
| | | Revisar virus | |

---

## Clasificación en 3 Niveles: Servicio → Sub-servicio → Tipo de Tarea

### 1. CORREO ELECTRÓNICO (66)

| Nivel 2 | Nivel 3 | Ejemplo | Tiempo |
|---------|---------|---------|---------|
| **Configuración** | Crear cuenta | crear correo erika.diaz@camposreyeros.com | 30m |
| | Configurar en equipo | configurar correo en equipo especifico | 30m |
| | Configurar en teléfono | ayudar a configurar correo en telefono | 30m |
| | Crear firma | crear firma de correo | 30m |
| | Configurar Spark | configurar spark | 30m |
| **Reparación** | No envía/recibe | mi correo no funciona, no envia ni recibe | 1h |
| | Pide clave | mi correo me pide clave | 30m |
| | No abre | correo no abre | 1h |
| | No conecta | no conecta al servidor de vata | 1h |
| **Mantenimiento** | Bandeja llena | mi correo esta lleno, bandeja al 95% | 15m |
| | Vaciar bandeja | vaciar correo resurtidos.her@cam | 15m |
| | Revisar cola | revisar cola de correo | 1hr |
| | Compresión respaldos | compresion de respaldos de correo | 2dias |
| **Gestión** | Corte de correo | corte de correo (varias) | 1h |
| | Recuperar correo | recuperar correo arizbet | 2dias |
| | Seguimiento | seguimiento de correo | 2h |
| **Seguridad** | Sospecha virus | oye crees que este correo sea virus? | 15m |
| | Correo mal configurado | reportar correo que esta mal configurado | 30m |

---

### 2. ACCESOS Y CREDENCIALES (21)

| Nivel 2 | Nivel 3 | Ejemplo |
|---------|---------|---------|
| **Datos de acceso** | Servidor | facilitar datos de acceso al servidor "revisarespgdl" | 30m |
| | Equipo | facilitar datos de acceso al equipo de cursos | 30m |
| | CRM | facilitar datos de acceso a CRM (varias) | 15m |
| | Zoho | pedir acceso para x para zoho | 6h |
| **Contraseñas** | Correo | PONER LA CLAVE DE CORREO EN SPARK | 30m |
| | CRM | poner clave de CRM | 30m |
| | Servicio | facilitar contraseña de lomas del sol crm | 15m |
| **Permisos** | Base de datos | dar permiso a tabla r_friedman_cab | 30m |
| | Servidor | permisos servidor CEDIS de Canovas | 30m |

---

### 3. SOPORTE CONTPAQ (9)

| Nivel 2 | Nivel 3 | Ejemplo |
|---------|---------|---------|
| **Actualización** | Contabilidad | actualizar contpaq contabilidad | 1h |
| | Rebsamen | actualizar contpaq pco y rebsamen | dias |
| | Vata/Sep | actualizar contpaq vata y sep | 1dia |
| **Instalación** | Servidor | levantar servidor contpaq | 15m |
| | Estación | instalar contpaq | 1h |
| **Reparación** | No funciona | el remoto gris no abre contpaq contabilidad | 30m |
| | Licencia | se termino la licencia de contpaq facturacion | 6h |
| | Error general | No me deja entrar al Contpaq | 1h |

---

### 4. COMPRAS Y GESTIÓN (21)

| Nivel 2 | Nivel 3 | Ejemplo |
|---------|---------|---------|
| **Pedidos** | Equipos | pedido : LX-350, 2 OPTIPLEX 7010 SFF |
| | Componentes | pedido : EC-PM-80330 |
| **Cotizaciones** | Equipos | cotizar switch para distribucion |
| | Seguimiento | seguimiento cotizacion mettler toledo |
| **Licencias** | Windows | BUEN DIA SERA QUE ME PODRIAS APOYAR CON LA LICENCIA DE WINDO |
| **Seguimiento** | Pedidos | revisar requisicion cc |
| | Proveedores | seguimiento a corte de colorante |
| **Sistemas** | Sicrop | revisar pedidos sicrop |
| | Webcomex | instalacion de webcomex |
| | Importador | seguimiento: importador corob |
| **Gestión integra** | General | gestion integra |

---

### 5. SEGUIMIENTO Y ASESORÍA (18)

| Nivel 2 | Nivel 3 | Ejemplo |
|---------|---------|---------|
| **Revisión** | Equipo | revisar magno 2, revisar equipo vs virus |
| | Incidencia | revisar incidencia de hugo/luis |
| | Tema general | revision de escrito |
| **Seguimiento** | Proyecto | seguimiento equipo cedisxal |
| | Cotización | seguimiento cotizacion |
| | Proveedor | seguimiento: importador corob |
| **Asesoría** | Correo | oye pregunta crees que este correo sea virus? |
| | General | Te acuerdas de los datos de spark de Elizabeth? |

---

### 6. GESTIÓN DE PERSONAL (15)

| Nivel 2 | Nivel 3 | Ejemplo |
|---------|---------|---------|
| **Horarios** | Colocar | poner horario a hugo para este semana |
| | Revisar | revisar horarios del area de cc |
| | Corregir | revisar horario, que se resetea a uno incorrecto |
| **Vacaciones** | Autorizar | autorizar las vacaciones de neto |
| **Incidencias** | Revisar | revisar solicitud viaticos |
| **Checadas** | Ayuda | apoyo con checada, ayuda para checar salida |
| **Técnicos** | Gestión | tecnicos, checar horarios de tecnicos |

---

### 7. MANTENIMIENTO Y RESPALDO (13)

| Nivel 2 | Nivel 3 | Ejemplo |
|---------|---------|---------|
| **Respaldos** | Configurar | configurar rsync tapachula a respaldos |
| | Ejecutar | actualizar respaldos vallartat |
| | Recuperar | recurerar respaldo de hermosillo |
| | Revisar | revisar respaldos GTM |
| **Sincronización** | Rsync | desarrollo de rsync -precios, rsync de precios |
| | NAS | revisar sucursal xpuen que no se sincroniza con NAS |
| | Tablas | copiar tabla pords_ok de tlato a magno2 |
| **Almacenamiento** | Espacio | crear espacio en ds418, borrando respaldos |
| | Compresión | compresion de respaldos de correo |

---

### 8. PRUEBAS (13)

| Nivel 2 | Nivel 3 | Ejemplo |
|---------|---------|---------|
| **Sistema** | Prueba general | prueba (varias), qwe, xxxxxxxxxx |
| | Título ejemplo | titulo de ejemplo |
| | Mamamam | mamamam |

---

### 9. FALLAS Y ERRORES (9)

| Nivel 2 | Nivel 3 | Ejemplo |
|---------|---------|---------|
| **Conectividad** | No conecta | no conecta al servidor de vata |
| | Conexión fallida | revisar problematica de coneccio |
| **Software** | Error | falla en coreo, falla en saci de vata |
| | No funciona | no esta llegando respaldos de tapachula |
| **Hardware** | Equipo apagado | BUEN DIA el equipo de bcs esta apagado |
| | Daño | rompi vata, inundacion |

---

### 10. CAPACITACIÓN (8)

| Nivel 2 | Nivel 3 | Ejemplo |
|---------|---------|---------|
| **Cursos** | Preparativos | preparar captura de curso silvino |
| | Descargar material | descargar documentos del curso |
| **Video** | Capacitación | video capacitacion add y ezaudita |
| **General** | Capacitación CC | capacitacion cc |

---

### 11. SOPORTE GENERAL (23)

| Nivel 2 | Nivel 3 | Ejemplo |
|---------|---------|---------|
| **Misceláneo** | Tarea variada | apoyo, ayuda, xxxxxxxxxx |
| **Visita** | Atender | atender visita |
| **Remoto** | Cerrar | cerrar remoto |

---

### 12. DESARROLLO (7)

| Nivel 2 | Nivel 3 | Ejemplo |
|---------|---------|---------|
| **Sistema** | Crear | crear un sistema, crear un sistema de gestion |
| **Web** | Página | desarrollo de www con richy |
| **API** | Monitor | crear API para monitor |
| **Base datos** | PostgreSQL | crear spark de carlos.rodriguez |

---

### 13. RED Y CONECTIVIDAD (6)

| Nivel 2 | Nivel 3 | Ejemplo |
|---------|---------|---------|
| **Internet** | No llega | visita a almacen para revisar internet |
| | General | internet distribucion |
| **WiFi** | Acceso | acceso a wifi |
| **LAN** | Conexión | apoyar a cambio de red almacen a red vata |
| | Unidad red | ayudar a que funcioen la unidad de red |
| **NAS** | Sincronización | revisar sucursal xpuen que no se sincroniza |

---

### 14. CONFIGURACIÓN (6)

| Nivel 2 | Nivel 3 | Ejemplo |
|---------|---------|---------|
| **Equipo** | Office | activar office |
| | Teclado | como activar el teclado |
| **Servicio** | TeamViewer | que se agregue al teamviewer la cc de cozumel |
| | Spark | configurar spark, configurar spark y correo |
| | General | configurar correo extr |

---

### 15. INSTALACIÓN (5)

| Nivel 2 | Nivel 3 | Ejemplo |
|---------|---------|---------|
| **Software** | Contpaq | instalar contpaq |
| | Office | activando office con metodo power shell |
| | Agente | instalar agente de bryan |
| **Hardware** | Impresora | instalacion de impresora dymo |
| | Terminal | instalar terminal nominas |

---

### 16. CÁMARAS Y SEGURIDAD (4)

| Nivel 2 | Nivel 3 | Ejemplo |
|---------|---------|---------|
| **Cámaras** | Conteo | conteo por camara |
| | Retroalimentación | Dar retroalimentacion del fallo de las camras |
| **Cortinas** | Autorización | autorizar cortina M4 suc centro |
| | Reparación | seguimiento de cortina dañada |

---

### 17. CRM/ZOHO (3)

| Nivel 2 | Nivel 3 | Ejemplo |
|---------|---------|---------|
| **CRM** | Datos acceso | facilitar datos de CRM |
| **Zoho** | Explicación | explicar zoho |
| | Instalación | pedir acceso para zoho |

---

### 18. DOMINIO Y HOSTING (3)

| Nivel 2 | Nivel 3 | Ejemplo |
|---------|---------|---------|
| **Dominio** | Renovación | renovacion de dominio camposreyeros.com |
| **Hosting** | Recuperar | recuperar dominio aplicare.com.mx |

---

### 19. SERVIDORES Y BD (3)

| Nivel 2 | Nivel 3 | Ejemplo |
|---------|---------|---------|
| **Servidor** | Encender | encender servidor respaldos |
| **Base datos** | Permisos | dar permiso a tabla r_friedman_cab |
| | Conexión | no conecta a base de datos |

---

### 20. FACTURACIÓN/SAT (3)

| Nivel 2 | Nivel 3 | Ejemplo |
|---------|---------|---------|
| **XML** | En línea | xml en linea de brenda |
| **Declaraciones** | Personas Morales | Declaración Anual Personas Morales |
| | RESICO | delcracion de personas morales RESICO |

---

### 21. OFFICE (3)

| Nivel 2 | Nivel 3 | Ejemplo |
|---------|---------|---------|
| **Excel** | Ayuda | ayuda con excell |
| **Word** | Ayuda | ayuda con word |
| **General** | Activar | activar office |

---

### 22. ANTIVIRUS (2)

| Nivel 2 | Nivel 3 | Ejemplo |
|---------|---------|---------|
| **Deshabilitar** | Equipo específico | deshabilitar antivirus e equipo de karla cortina/betsabe/gabriela/lupita |
| **Excepciones** | Página web | hacer esepcion en el antivirus para que pase una pagina |

---

### 23. IMPRESORAS (1)

| Nivel 2 | Nivel 3 | Ejemplo |
|---------|---------|---------|
| **Tickets** | Instalar | impresoras de ticket |
| **Dymo** | Instalar | instalacion de impresora dymo |
| **General** | Revisar | revisar instalar ipresora |

---

## Resumen de la Estructura

| Nivel 1 (Servicio) | Nivel 2 (Sub-servicio) | Nivel 3 (Tipo tarea) | Total |
|--------------------|----------------------|---------------------|-------|
| Correo Electrónico | 5 | 17 | 66 |
| Accesos/Credenciales | 3 | 8 | 21 |
| Compras/Gestión | 6 | 11 | 21 |
| Soporte General | 3 | 5 | 23 |
| Seguimiento/Asesoría | 3 | 8 | 18 |
| Gestión Personal | 5 | 7 | 15 |
| Mantenimiento/Respaldo | 3 | 9 | 13 |
| Pruebas | 2 | 5 | 13 |
| Soporte Contpaq | 3 | 9 | 9 |
| Fallas/Errores | 3 | 6 | 9 |
| Capacitación | 3 | 4 | 8 |
| Desarrollo | 4 | 5 | 7 |
| Red/Conectividad | 5 | 6 | 6 |
| Configuración | 2 | 5 | 6 |
| Instalación | 2 | 5 | 5 |
| Cámaras/Seguridad | 2 | 4 | 4 |
| CRM/Zoho | 2 | 3 | 3 |
| Dominio/Hosting | 2 | 2 | 3 |
| Servidores/BD | 2 | 3 | 3 |
| Facturación/SAT | 3 | 3 | 3 |
| Office | 3 | 3 | 3 |
| Antivirus | 2 | 2 | 2 |
| Impresoras | 3 | 3 | 1 |

**Totales:** 23 servicios → ~65 sub-servicios → ~150 tipos de tarea
